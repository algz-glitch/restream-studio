...
export function Logs({ items, loading, error, hasMore, onMore, onRetry }: Props) {
  ...
  return <section className="panel logs" role="region" aria-labelledby="logs-title"><div className="panel__heading"><div><h2 id="logs-title">运行日志</h2><p>敏感字段在显示前再次脱敏</p></div><div className="logs-tools"><label className="filter">日志类别<select value={filter} onChange={(e) => setFilter(e.target.value)}><option value="all">全部</option><option value="info">信息</option><option value="warning">警告</option><option value="error">错误</option></select></label><button className="button" onClick={onRetry}>刷新日志</button></div></div>
