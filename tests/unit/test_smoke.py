...
def test_package_exposes_version() -> None:
    ...
    assert restream_studio.__version__ == pyproject_version == package_version == "0.1.2"
...
def test_runtime_version_uses_valid_bundled_metadata(
    ...
    metadata.write_text("0.1.2", encoding="ascii")
    monkeypatch.setattr(restream_studio, "_BUNDLED_VERSION", metadata)

    assert restream_studio._runtime_version() == "0.1.2"
...
def test_runtime_version_rejects_invalid_bundled_metadata(
    ...
    metadata = tmp_path / "version.txt"
    metadata.write_text("0.1.2-dev", encoding="ascii")
    monkeypatch.setattr(restream_studio, "_BUNDLED_VERSION", metadata)
